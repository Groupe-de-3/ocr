...
DougBackendWindow *doug_backend_window_new(
    DougBackendConnection *connection, DougWindowProperties props
) {
    xcb_connection_t *conn = connection->conn;

    xcb_screen_t *screen =
        xcb_setup_roots_iterator(xcb_get_setup(connection->conn)).data;
    ...
    xcb_create_window(
        conn, screen->root_depth, window, screen->root, (short)props.position.x,
        (short)props.position.y, (unsigned short)props.size.x,
        (unsigned short)props.size.y, 1, XCB_WINDOW_CLASS_INPUT_OUTPUT,
    ...
    xcb_icccm_size_hints_set_position(
        &size_hints, true, props.position.x, props.position.y
    );
    xcb_icccm_size_hints_set_size(
        &size_hints, true, props.size.x, props.size.y
    );
    xcb_icccm_size_hints_set_min_size(
        &size_hints, props.min_size.x, props.min_size.y
    );
    xcb_icccm_size_hints_set_max_size(
        &size_hints, props.max_size.x, props.max_size.y
    );
...
void doug_backend_window_destroy(
    DougBackendWindow *window, bool disconnect_if_possible
) {
...
DougBackendConnection *
doug_backend_window_get_connection(DougBackendWindow *window) {
    return window->conn;
}

static uint8_t
get_size_hints(DougBackendWindow *window, xcb_size_hints_t *hints) {
    return xcb_icccm_get_wm_normal_hints_reply(
        window->conn->conn,
        xcb_icccm_get_wm_normal_hints_unchecked(
            window->conn->conn, window->window
        ),
        hints, NULL
    );
}
...
void doug_backend_window_set_size(DougBackendWindow *window, ipoint2d_t size) {
    ...
    xcb_icccm_size_hints_set_size(&hints, true, size.x, size.y);
    xcb_icccm_set_wm_normal_hints(window->conn->conn, window->window, &hints);
}
void doug_backend_window_set_min_size(
    DougBackendWindow *window, ipoint2d_t min_size
) {
    xcb_size_hints_t hints;
    uint8_t          result = get_size_hints(window, &hints);
    if (result != 0)
    ...
    xcb_icccm_size_hints_set_min_size(&hints, min_size.x, min_size.y);
    xcb_icccm_set_wm_normal_hints(window->conn->conn, window->window, &hints);
}
void doug_backend_window_set_max_size(
    DougBackendWindow *window, ipoint2d_t max_size
) {
    xcb_size_hints_t hints;
    uint8_t          result = get_size_hints(window, &hints);
    if (result != 0)
    ...
    xcb_icccm_size_hints_set_max_size(&hints, max_size.x, max_size.y);
    xcb_icccm_set_wm_normal_hints(window->conn->conn, window->window, &hints);
}
void doug_backend_window_set_title(
    DougBackendWindow *window, const char *title
) {
    xcb_icccm_set_wm_name(
        window->conn->conn, window->window,
        doug_natom(window->conn->conn, "STRING"), 0, (uint32_t)strlen(title),
        title
    );
}
