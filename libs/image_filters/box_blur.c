...
static void
box_blur_horizontal(ImageView *in, ImageView *out, size_t box_size) {
    int half_size = (int)box_size / 2;

    enum PixelFormat target_format =
        img_equivalent_float_channels_format(in->image->format);
    ...
    for (int y = 0; y < in->height; y++) {
        ...
        for (int x = -half_size; x <= half_size; x++) {
            any_pixel_t v =
                img_some_to_any(imgv_get_pixel_some(in, x, y), target_format);
        ...
        for (int x = 1; x < in->width; x++) {
            any_pixel_t before_v = img_some_to_any(
                imgv_get_pixel_some(in, x - half_size - 1, y), target_format
            );
            for (size_t c = 0; c < channel_length; c++)
                new_value[c] -= before_v.float_channels[c];

            any_pixel_t after_v = img_some_to_any(
                imgv_get_pixel_some(in, x + half_size, y), target_format
            );
            for (size_t c = 0; c < channel_length; c++)
...
static void box_blur_vertical(ImageView *in, ImageView *out, size_t box_size) {
    ...
    for (int x = 0; x < in->width; x++) {
        ...
        for (int y = 1; y < in->height; y++) {
            any_pixel_t above_v = img_some_to_any(
                imgv_get_pixel_some(in, x, y - half_size - 1), target_format
            );
            for (size_t c = 0; c < channel_length; c++)
                new_value[c] -= above_v.float_channels[c];

            any_pixel_t below_v = img_some_to_any(
                imgv_get_pixel_some(in, x, y + half_size), target_format
            );
            for (size_t c = 0; c < channel_length; c++)
...
void box_blur_run(ImageView *in, ImageView *out, size_t box_size) {
    ...
    Image h_buffer =
        img_new((size_t)out->width, (size_t)out->height, out->image->format);
    ...
    enum PixelFormat target_format =
        img_equivalent_float_channels_format(out->image->format);
    size_t channel_length = img_float_channels_length(target_format);
