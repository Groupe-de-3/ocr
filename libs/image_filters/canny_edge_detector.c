...
static void extend_edges(
    ImageView *canny, ImageView *gradient_directions, float angle_limit
) {
    ...
    for (int x = 0; x < w; x++) {
        for (int y = 0; y < h; y++) {
            while (vec_size(pixels) > 0) {
                ...
                for (int dy = -1; dy <= 1; dy++) {
                    ...
                    for (int dx = -1; dx <= 1; dx++) {
                        ...
                        // % for negative value isn't like a mathematic
                        // remainder
                        px       = ((px % w) + w) % w;
                        py       = ((py % h) + h) % h;
                        double g = imgv_get_pixel_grayscale(canny, px, py);
                        double d = imgv_get_pixel_grayscale(
                            gradient_directions, px, py
                        );
                        double angle_dist =
                            fmod(fabs(d - pixel.strong_direction), 2 * M_PI);
                        if (angle_dist > M_PI)
                            angle_dist = 2 * M_PI - angle_dist;
                        if (angle_dist < angle_limit &&
            ...
            if (feq(imgv_get_pixel_grayscale(canny, x, y), 0.5)) {
                double direction =
                    imgv_get_pixel_grayscale(gradient_directions, x, y);
                *vec_push(&pixels, struct Pixel) = (struct Pixel){
                    .x                = x,
                    .y                = y,
...
void canny_run(
    ImageView *gradient, ImageView *gradient_direction, ImageView *out,
    CannyParameters parameters
) {
    for (int y = 0; y < gradient_direction->height; y++) {
        for (int x = 0; x < gradient_direction->width; x++) {
            ...
            float left_gradient =
                (float)imgv_get_pixel_grayscale(gradient, x + dx, y + dy);
        ...
        for (int x = 0; x < out->width; x++) {
            ...
            if (v < 0.75 && v > 0.25)
                imgv_set_pixel_grayscale(
                    out, x, y, parameters.output_value_for_soft_edges
                );
        }
    }
}
