...
Matrix(void) m_create(size_t elem_size, size_t dimc, size_t dimv[]) {
    ...
    MatrixMetadata *matrix = malloc(sizeof(MatrixMetadata) + elem_size * total_size);
    ...
    for (size_t row_i = 0; row_i < m_width(a); row_i++) { \
        for (size_t col_i = 0; col_i < m_height(b); col_i++) { \
            ...
            for (size_t row_and_col = 0; row_and_col < c1; row_and_col++) \
                sum += a[row_and_col + row_i * c1] * b[col_i + row_and_col * c2]; \
            out[col_i + row_i * c2] = sum; \
        } \
    } \
}
matrix_mul(m_imul, int)
matrix_mul(m_fmul, double)
matrix_mul(m_fmulf, float)
...
void name(Matrix(type) a, Matrix(type) out) { \
 ...
 \
    ...
    for (;;) { \
        ...
        for ( \
            ssize_t dim_i = (ssize_t)meta_a->dimc-1; \
            dim_i > 0 /* Not processing last one*/; \
            dim_i-- \
        ) { \
