...
#define h_assert_msg(expr, ...) \
  (expr) ? \
     (void)0 : \
     _h_assert_failed(__FILE__, __LINE__, "Assertion '"#expr"' failed", __VA_ARGS__)
...
#define _h_assert_op(TYPE, FORMAT_FLAGS, X, OP, Y)\
    int _h_x = (X); \
    int _h_y = (Y); \
    h_assert_msg(_h_x OP _h_y, "Assertion '%s' failed: %s == %"FORMAT_FLAGS", %s == %"FORMAT_FLAGS, #X" "#OP" "#Y, #X, _h_x, #Y, _h_y); \
} while (0)
